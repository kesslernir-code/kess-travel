  ...
  document.getElementById('confirmBtn').addEventListener('click', async function() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // an iframe inside the dashboard's own tab-2 (renderDashboard embeds
        // every tab). Navigating plain window only redirects that iframe,
        // which then loads the WHOLE dashboard recursively inside itself --
        // a second full header+tab-bar rendered inside the first, defaulting
        // to tab 1, stacked directly under the real one. top is the
        // outermost window regardless of nesting, and is safely just window
        // itself on a direct, non-embedded page load.
